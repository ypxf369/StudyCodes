...
public class MyDispatcherServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
            ...
            resp.getWriter().write("500 Service internal error!");
        }
    }
    ...
    private void doDispatch(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        //利用反射机制来调用
        try {
            method.invoke(this.controllerMap.get(url), paramValues);//第一个参数时method所对应的实例，在ioc容器中
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void doLoadConfig(String location) {
        //把web.xml中的contextConfigLocation对应value值得文件加载到流里面
        InputStream resourceAsStream = this.getClass().getClassLoader().getResourceAsStream(location);
        try {
            //用Properties对象加载文件里面的内容
    ...
    private void doInstance() {
        ...
        for (String className : classNames) {
            try {
                //把类搞出来，反射来实例化（只有家@MyController需要实例化）
