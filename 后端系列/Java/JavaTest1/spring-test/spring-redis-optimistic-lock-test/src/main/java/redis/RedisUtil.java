...
public class RedisUtil {
    ...
    public RedisUtil() {
        if (pool == null) {
            ...
            // 如果赋值为-1，则表示不限制；如果pool已经分配了maxActive个jedis实例，则此pool的状态为exhausted（耗尽）
            config.setMaxTotal(10000);
            // 控制一个pool最多有多少个状态为idle(空闲的)的jedis实例
            config.setMaxIdle(2000);
            // 表示当borrow(引入)一个jedis实例时，最大的等待时间，如果超过等待时间则抛出JedisConnectionException；
    ...
    /**
     * <p>
     * 通过key 对value进行加值+1操作,当value不是int类型时会返回错误,当key不存在时则value为1
     * </p>
     *
     * @param key
     * @return 加值后的结果
